[lumen-advanced (3).html](https://github.com/user-attachments/files/25569669/lumen-advanced.3.html)
<!DOCTYPE html>
<html lang="vi">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>LUMEN — Hệ thống Phát hiện Sớm Cảm xúc</title>
...
<style>
/* ============================================================
   LUMEN ADVANCED — CSS FOUNDATION
   Aesthetic: Deep Navy × Cream × Terracotta × Sage
   ============================================================ */
:root {
  --navy:    #0e1117;
  --navy2:   #141924;
...
<nav>
  ...
  <div class="nav-tabs">
    <button class="nav-tab active" onclick="gotoScreen('home',this)">Trang chủ</button>
    <button class="nav-tab" onclick="gotoScreen('chat',this)">Chia sẻ</button>
    <button class="nav-tab" onclick="gotoScreen('dashboard',this)">Tổng quan</button>
    <button class="nav-tab" onclick="gotoScreen('timeline',this)">Dòng cảm xúc</button>
    <button class="nav-tab" onclick="gotoScreen('school',this)">Cổng trường học</button>
    <button class="nav-tab" onclick="gotoScreen('settings',this)">Cài đặt</button>
    <button class="nav-tab" onclick="toggleKidsMode(this)" id="kidsNavTab" style="background:linear-gradient(135deg,rgba(255,179,71,0.2),rgba(255,126,95,0.2));border:1px solid rgba(255,179,71,0.3);color:#f0a060;">🧸 Trẻ em</button>
  </div>
  <div class="nav-actions">
    <div class="icon-btn" onclick="openVoice()" title="Nhận dạng giọng nói thực (Web Speech API)">🎙️</div>
    <div class="icon-btn" onclick="toggleTheme()" title="Đổi theme" id="themeBtn">🌙</div>
  </div>
</nav>
...
<section class="screen active" id="screen-home">
  <div class="home-inner">
    ...
    <div class="home-eyebrow">Hệ thống phát hiện sớm cảm xúc · AI nhân văn</div>
    <h1 class="home-title">Chào bạn.<br><em>Hôm nay bạn có thật sự ổn không?</em></h1>
    <p class="home-sub">LUMEN lắng nghe. Không phán xét. Không chẩn đoán.<br>Chỉ đồng hành cùng bạn trước khi bạn cần.</p>
    <div class="home-btns">
      <button class="home-btn home-btn-primary" onclick="startChat('share')">
        <span class="btn-emoji">🌿</span>
        <div class="btn-text-wrap">
          <span class="btn-label">Tôi muốn chia sẻ</span>
          <span class="btn-desc">Kể cho LUMEN nghe điều bạn đang cảm thấy</span>
        </div>
      </button>
      <button class="home-btn home-btn-secondary" onclick="startChat('unsure')">
        <span class="btn-emoji">🌊</span>
        <div class="btn-text-wrap">
          <span class="btn-label">Tôi không chắc mình cảm thấy gì</span>
          <span class="btn-desc">Để LUMEN giúp bạn nhận ra</span>
        </div>
      </button>
      <button class="home-btn home-btn-ghost" onclick="gotoScreen('dashboard',null)">
        <span class="btn-emoji">🌙</span>
        <div class="btn-text-wrap">
          <span class="btn-label">Tôi chỉ muốn xem</span>
          <span class="btn-desc">Xem dashboard tổng quan</span>
        </div>
      </button>
    </div>
  </div>
  ...
  <div class="chat-layout">
    <div class="chat-main">
      ...
      <div class="chat-input-area">
        <textarea id="chatInput" placeholder="Chia sẻ cảm xúc của bạn… LUMEN đang lắng nghe." rows="3"
          oninput="autoResize(this)" onkeydown="onChatKey(event)"></textarea>
        <div class="input-bar">
          <div class="input-hints">
            <div class="hint-chip" onclick="injectHint('Dạo này tôi cảm thấy mệt lắm')">Tôi mệt</div>
            <div class="hint-chip" onclick="injectHint('Tôi ổn thôi, không có gì đáng nói')">Tôi ổn</div>
            <div class="hint-chip" onclick="injectHint('Tôi không biết mình đang cảm thấy gì nữa')">Không biết</div>
          </div>
    ...
    <div class="chat-sidebar">
      <div class="sidebar-card">
        ...
        <div class="score-ring-wrap">
          <div class="score-ring">
            ...
            <div class="ring-center">
              <div class="ring-score" id="sideScore">—</div>
        ...
        <div class="risk-badge risk-stable" id="riskBadge">
          ...
          <span id="riskText">Chưa phân tích</span>
        </div>
      </div>

      <div class="sidebar-card">
        <h4>Patterns phát hiện</h4>
        <div class="pattern-grid">
          <div class="pat-item" id="pat-self_negation"><span class="pat-icon">🔄</span>Tự phủ nhận cảm xúc</div>
          <div class="pat-item" id="pat-emotional_suppression"><span class="pat-icon">🤐</span>Kìm nén cảm xúc</div>
          <div class="pat-item" id="pat-withdrawal_language"><span class="pat-icon">🚪</span>Ngôn ngữ thu mình</div>
          <div class="pat-item" id="pat-fatigue_signal"><span class="pat-icon">🪫</span>Kiệt sức</div>
          <div class="pat-item" id="pat-self_blame"><span class="pat-icon">⚖️</span>Tự trách bản thân</div>
        </div>
      </div>

      <div class="sidebar-card">
        <h4>Lịch sử phiên này</h4>
        <div class="mini-timeline" id="miniTimeline">
          <div style="font-size:0.78rem;color:var(--cream3);opacity:0.6;text-align:center;padding:8px 0;">Chưa có lịch sử</div>
        </div>
      </div>
      ...
      <div style="padding:14px 16px;background:rgba(196,115,90,0.06);border:1px solid var(--border2);border-radius:var(--r);font-size:0.72rem;color:var(--cream3);line-height:1.7;">
        ⚠️ <strong style="color:var(--cream2)">LUMEN</strong> là công cụ hỗ trợ sớm. Không thay thế chuyên gia tâm lý.
      </div>
    </div>
  </div>
</section>
...
<section class="screen" id="screen-dashboard">
  <div class="dashboard-grid">
    <div class="dash-card">
      <h3>📊 Điểm cảm xúc trung bình</h3>
      <div class="stat-big" id="dashAvg">72</div>
      <div class="stat-sub">Tuần này</div>
      <div class="stat-trend trend-down">↓ 8 điểm so với tuần trước</div>
    </div>
    <div class="dash-card">
      <h3>🔁 Lần tự trấn an</h3>
      <div class="stat-big" id="dashReassure">14</div>
      <div class="stat-sub">Lần nói "Tôi ổn" / "Không sao"</div>
      <div class="stat-trend trend-up">↑ Cao hơn bình thường</div>
    </div>
    <div class="dash-card">
      <h3>🌿 Chuỗi ổn định</h3>
      <div class="stat-big">3</div>
      <div class="stat-sub">ngày liên tiếp score ≥ 60</div>
      <div class="stat-trend trend-flat">→ Ổn định</div>
    </div>
    ...
    <div class="dash-card">
      <h3>🤐 Silence Pattern</h3>
      <div class="silence-bars">
        <div class="silence-bar-row">
          <div class="silence-bar-label">Câu trả lời ngắn</div>
        ...
        <div class="silence-bar-row">
          <div class="silence-bar-label">Phủ nhận cảm xúc</div>
        ...
        <div class="silence-bar-row">
          <div class="silence-bar-label">Tự trách</div>
        ...
        <div class="silence-bar-row">
          <div class="silence-bar-label">Thu mình</div>
    ...
    <div class="dash-card full">
      <h3>💬 Tooltip AI: Phát hiện lệch pha</h3>
      <div style="background:var(--navy3);border:1px solid var(--border2);border-radius:var(--rs);padding:16px 20px;font-family:'Playfair Display',serif;font-style:italic;font-size:1rem;color:var(--cream2);line-height:1.8;">
        "Tuần này bạn đã nói <span style="color:var(--terra2)">'Tôi ổn'</span> nhiều hơn bình thường — nhưng điểm năng lượng của bạn lại thấp hơn 20%. LUMEN nhận thấy có thể bạn đang cố gắng tỏ ra ổn định với người xung quanh nhiều hơn mức bạn thực sự cảm thấy."
      </div>
      <div style="margin-top:12px;font-size:0.8rem;color:var(--cream3);">Phân tích dựa trên 7 ngày gần nhất · Ẩn danh · Không lưu định danh</div>
    </div>
  </div>
</section>
...
<section class="screen" id="screen-timeline">
  <div class="timeline-wrap">
    <div class="timeline-header">
      <div>
        <h2>Dòng cảm xúc</h2>
        <p>Biểu đồ sóng cảm xúc 14 ngày gần nhất</p>
      </div>
      <div style="display:flex;gap:10px;">
        <div class="hint-chip" onclick="redrawChart('2w')">2 tuần</div>
        <div class="hint-chip" onclick="redrawChart('1m')">1 tháng</div>
        <div class="hint-chip" onclick="redrawChart('3m')">3 tháng</div>
      </div>
    </div>
    ...
    <div style="margin-top:28px;">
      <div class="dash-card" style="background:var(--navy2);">
        <h3>📌 Phát hiện "Im lặng kéo dài"</h3>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-top:4px;">
          <div style="text-align:center;padding:16px;">
            ...
            <div style="font-size:0.75rem;color:var(--cream3);margin-top:4px;">ngày không chia sẻ</div>
          </div>
          <div style="text-align:center;padding:16px;">
            <div style="font-family:'Playfair Display',serif;font-size:2rem;color:var(--amber);">×6</div>
            <div style="font-size:0.75rem;color:var(--cream3);margin-top:4px;">lần tự trấn an liên tiếp</div>
          </div>
          <div style="text-align:center;padding:16px;">
            <div style="font-family:'Playfair Display',serif;font-size:2rem;color:var(--sage);">↓18</div>
            <div style="font-size:0.75rem;color:var(--cream3);margin-top:4px;">điểm drift trong 5 ngày</div>
          </div>
        </div>
      </div>
    </div>
...
<section class="screen" id="screen-school">
  <div class="school-wrap">
    <h2 style="font-family:'Playfair Display',serif;font-size:1.8rem;color:var(--cream);margin-bottom:8px;">Cổng trường học ẩn danh</h2>
    <p style="font-size:0.85rem;color:var(--cream3);margin-bottom:20px;">Dữ liệu tổng hợp, ẩn danh — dành cho giáo viên/tư vấn trường.</p>

    <div class="anon-notice">
      <span style="font-size:1.3rem;">🛡</span>
      <div>Tất cả dữ liệu được <strong>ẩn danh hoàn toàn</strong>. Không có thông tin định danh cá nhân. Chỉ hiển thị xu hướng tổng thể của nhóm.</div>
    </div>

    <div style="margin-bottom:20px;font-size:0.7rem;letter-spacing:0.2em;text-transform:uppercase;color:var(--terra);">Tổng quan lớp học</div>
    <div class="class-grid">
      <div class="class-card" onclick="classClick(this,'10A1','Cao')">
        <div class="class-num">87</div>
        <div class="class-label">Điểm TB · 10A1</div>
        <div class="class-bar"><div class="class-bar-fill" style="width:87%;background:var(--sage);"></div></div>
      </div>
      <div class="class-card" onclick="classClick(this,'10A2','Trung bình')">
        <div class="class-num" style="color:var(--amber)">62</div>
        <div class="class-label">Điểm TB · 10A2</div>
        <div class="class-bar"><div class="class-bar-fill" style="width:62%;background:var(--amber);"></div></div>
      </div>
      <div class="class-card" onclick="classClick(this,'11B1','Thấp')">
        <div class="class-num" style="color:var(--rose)">38</div>
        <div class="class-label">Điểm TB · 11B1</div>
        <div class="class-bar"><div class="class-bar-fill" style="width:38%;background:var(--rose);"></div></div>
      </div>
      <div class="class-card" onclick="classClick(this,'11B2','Trung bình')">
        <div class="class-num">71</div>
        <div class="class-label">Điểm TB · 11B2</div>
        <div class="class-bar"><div class="class-bar-fill" style="width:71%;background:var(--terra);"></div></div>
      </div>
    </div>

    <div class="dash-card" style="background:var(--navy2);">
      <h3>⚠️ Cảnh báo xu hướng (ẩn danh)</h3>
      <div style="display:flex;flex-direction:column;gap:10px;margin-top:4px;">
        <div style="display:flex;align-items:center;gap:12px;padding:12px;background:rgba(196,90,106,0.08);border:1px solid rgba(196,90,106,0.2);border-radius:var(--rs);">
          ...
          <div>
            <div style="font-size:0.84rem;color:var(--cream2);font-weight:500;">Lớp 11B1 — Điểm cảm xúc trung bình giảm 22 điểm trong 2 tuần</div>
            <div style="font-size:0.75rem;color:var(--cream3);margin-top:2px;">Khuyến nghị: Tăng cường hoạt động kết nối nhóm · Theo dõi sát hơn</div>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:12px;padding:12px;background:rgba(212,168,75,0.08);border:1px solid rgba(212,168,75,0.2);border-radius:var(--rs);">
          <span style="font-size:1.1rem;">🟡</span>
          <div>
            <div style="font-size:0.84rem;color:var(--cream2);font-weight:500;">Lớp 10A2 — Tần suất "Tôi ổn" cao bất thường trong 5 ngày</div>
            <div style="font-size:0.75rem;color:var(--cream3);margin-top:2px;">Khuyến nghị: Tạo không gian chia sẻ an toàn trong lớp</div>
          </div>
        </div>
      </div>
    </div>
...
<section class="screen" id="screen-settings">
  <div class="settings-wrap">
    <h2>Tùy chỉnh LUMEN</h2>
    <p>Cá nhân hóa trải nghiệm của bạn. Mọi thay đổi được lưu cục bộ, không gửi lên server.</p>

    <div class="settings-group">
      <h3>Giao diện</h3>
      <div class="setting-row">
        <div class="setting-info">
          <div class="setting-label">Chủ đề màu sắc</div>
          <div class="setting-desc">Chọn gam màu phù hợp với tâm trạng bạn</div>
        </div>
      ...
      <div class="setting-row">
        <div class="setting-info">
          <div class="setting-label">Font chữ</div>
          <div class="setting-desc">Chọn font dễ đọc với bạn</div>
        </div>
        <div class="setting-control">
          <div class="font-opts">
    ...
    <div class="settings-group">
      ...
      <div class="setting-row">
        <div class="setting-info">
          <div class="setting-label">Hiệu ứng animation</div>
          <div class="setting-desc">Tắt nếu bạn nhạy cảm với chuyển động</div>
        </div>
        <div class="setting-control">
      ...
      <div class="setting-row">
        <div class="setting-info">
          <div class="setting-label">Nhạc nền thư giãn</div>
          <div class="setting-desc">Âm thanh binaural nhẹ khi sử dụng</div>
        </div>
        <div class="setting-control">
      ...
      <div class="setting-row">
        <div class="setting-info">
          <div class="setting-label">Tự động chế độ tối theo giờ</div>
          <div class="setting-desc">Bật dark mode tự động sau 19:00</div>
        </div>
        <div class="setting-control">
      ...
      <div class="setting-row">
        <div class="setting-info">
          <div class="setting-label">Hiệu ứng hạt nền</div>
          <div class="setting-desc">Tắt để tăng hiệu suất trên thiết bị cũ</div>
        </div>
        <div class="setting-control">
    ...
    <div class="settings-group">
      ...
      <div class="setting-row">
        <div class="setting-info">
          <div class="setting-label">Lưu lịch sử phiên</div>
          <div class="setting-desc">Lưu cục bộ trên thiết bị, không gửi server</div>
        </div>
        <div class="setting-control">
      ...
      <div class="setting-row">
        <div class="setting-info">
          <div class="setting-label" style="color:var(--rose)">Xóa tất cả dữ liệu cục bộ</div>
          <div class="setting-desc">Không thể khôi phục</div>
        </div>
        <div class="setting-control">
          <button style="padding:7px 16px;border-radius:50px;background:rgba(196,90,106,0.15);border:1px solid rgba(196,90,106,0.3);color:var(--rose);font-family:'Sora',sans-serif;font-size:0.78rem;cursor:pointer;" onclick="clearData()">Xóa dữ liệu</button>
        </div>
      </div>
    </div>
  </div>
...
<!-- KIDS MODE FAB -->
<div id="kidsModeBtn" onclick="toggleKidsMode(null)" title="Chế độ trẻ em">
  🧸
  <div class="kids-fab-tooltip">Chế độ Trẻ em</div>
</div>

<!-- RESULT OVERLAY -->
<div class="result-overlay" id="resultOverlay">
  <div class="result-panel" id="resultPanel">
    <div class="panel-close" onclick="closeResult()">✕</div>
    <div class="result-orb stable" id="resultOrb">🌿</div>
    <h2 id="resultTitle">Phân tích LUMEN</h2>
    ...
    <div class="urgent-block" id="urgentBlock">
      ...
      <div>
        <h5>Bạn không phải đi qua điều này một mình</h5>
        <p>LUMEN nhận thấy bạn đang mang rất nhiều thứ trong im lặng. Dũng cảm tìm kiếm hỗ trợ là sức mạnh thật sự.</p>
        <div class="hotline-num">📞 1800 599 920</div>
        <p style="font-size:0.72rem;opacity:0.6;margin-top:4px;">Miễn phí · 24/7 · Đường dây sức khỏe tâm thần</p>
      </div>
    </div>
    <div class="json-accordion">
      <div class="json-toggle" onclick="toggleJson()">
        <span style="font-size:0.9rem;">{}</span> Raw JSON output
        <span id="jsonArrow" style="margin-left:auto;">▾</span>
      </div>
...
<div class="hotline-bar" id="hotlineBar">
  <span>📞 Đường dây hỗ trợ khẩn cấp:</span>
  <strong>1800 599 920</strong>
  <span>· Miễn phí · 24/7</span>
  <button class="hotline-close" onclick="document.getElementById('hotlineBar').classList.remove('show')">Đóng</button>
</div>

<!-- VOICE MODAL -->
<div class="voice-modal" id="voiceModal">
  <button class="voice-close" onclick="closeVoice()">✕ Đóng</button>
  <div class="voice-orb"></div>
  <div id="voiceModalTitle" style="font-size:0.9rem;font-weight:500;color:var(--cream);margin-bottom:12px;">🎙️ Nhận dạng giọng nói</div>
  <div class="voice-text" id="voiceText">
    "Bạn không yếu đuối vì cảm thấy mệt.<br>Bạn chỉ đang mang nhiều thứ hơn bạn nghĩ."
  </div>
  <p style="font-size:0.78rem;color:rgba(245,240,232,0.35);margin-top:20px;">Chế độ giọng nói · Bấm bất kỳ đâu để đóng</p>
</div>

<script>
/* ============================================================
   LUMEN ADVANCED — JAVASCRIPT ENGINE
   ============================================================ */

// ── STATE (using let, NOT const for reassignable values) ──────
let state = {
  currentScreen: 'home',
  theme: 'dark',
  score: null,
...
function toggleTheme() {
  ...
  // ✅ FIX: redraw timeline canvas after theme change so colors update
  if (state.currentScreen === 'timeline') setTimeout(drawTimeline, 50);
}

// ── NAVIGATION — see patched version at bottom of file ────────
function gotoScreen_UNUSED(id, tabEl) {
  // legacy — replaced by patched version at bottom
...
// ── CHAT ENGINE ───────────────────────────────────────────────
const SYSTEM = `You are LUMEN AI – an empathetic early-warning emotional support system.
...
const recMap = {
  breathing_exercise: {icon:'🫁',title:'Bài thở 4-7-8',desc:'Hít 4s · Giữ 7s · Thở ra 8s. Lặp 4 vòng.'},
  journal_prompt: {icon:'✍️',title:'Viết nhật ký',desc:'5 phút mỗi tối — không cần hay, chỉ cần thật.'},
  connect_with_someone: {icon:'🤝',title:'Kết nối ai đó',desc:'Nhắn cho một người bạn tin tưởng.'},
  seek_professional_support_if_needed: {icon:'💬',title:'Gặp chuyên gia',desc:'Khi cần hỗ trợ chuyên sâu hơn.'},
  rest_and_self_care: {icon:'🌿',title:'Nghỉ ngơi',desc:'Cho phép mình dừng lại. Nghỉ ngơi là sức mạnh.'},
  mindful_moment: {icon:'🧘',title:'Khoảnh khắc tĩnh',desc:'1 phút — chỉ cảm nhận hơi thở.'},
};
...
function startChat(mode) {
  ...
  setTimeout(() => {
    if (mode === 'share') {
      addAiMsg('Chào bạn. Tôi ở đây lắng nghe. Hôm nay bạn đang cảm thấy thế nào?');
    } else {
      addAiMsg('Không biết mình đang cảm thấy gì — điều đó cũng bình thường lắm. Hãy thử với câu hỏi này nhé:');
...
function addColorQuestion() {
  const colors = [
    {c:'#4a7fb5',label:'Xanh dương',meaning:'bình yên'},
    {c:'#6a9e82',label:'Xanh lá',meaning:'hy vọng'},
    {c:'#d4a84b',label:'Vàng',meaning:'vui vẻ'},
    {c:'#c4735a',label:'Cam',meaning:'ấm nhưng mệt'},
    {c:'#8a78c4',label:'Tím',meaning:'u buồn'},
    {c:'#888',label:'Xám',meaning:'trống rỗng'},
    {c:'#2a2420',label:'Tối',meaning:'nặng nề'},
  ];
  const html = `<div style="font-size:0.88rem;color:var(--cream2);margin-bottom:10px;">Nếu cảm xúc hôm nay là một màu sắc, nó sẽ là…?</div>
    <div class="color-picker-msg">
      ${colors.map(c => `<div class="color-orb" style="background:${c.c}" title="${c.label} — ${c.meaning}" onclick="pickColor(this,'${c.label}','${c.meaning}')"></div>`).join('')}
    </div>`;
  addAiMsgRaw(html);
}
...
function pickColor(el, label, meaning) {
  ...
  addUserMsg(`Màu ${label} — ${meaning}`);
  setTimeout(() => {
    addAiMsg(`Màu ${label}... ${meaning}. Cảm ơn bạn đã chia sẻ điều đó. Bạn có muốn kể thêm không?`);
  }, 500);
}

function addScaleQuestion() {
  const html = `<div style="font-size:0.88rem;color:var(--cream2);margin-bottom:10px;">Năng lượng của bạn hôm nay từ 1–10?</div>
    <div class="scale-msg">
...
function pickScale(el, n) {
  ...
  el.classList.add('picked');
  addUserMsg(`Năng lượng: ${n}/10`);
  const resp = n <= 3 ? 'Năng lượng khá thấp. Bạn có đang ngủ đủ giấc không?' :
               n <= 6 ? 'Mức trung bình — không tệ nhưng cũng không tốt hẳn. Có điều gì đang kéo bạn xuống không?' :
               'Khá ổn! Điều gì đang giúp bạn duy trì được năng lượng vậy?';
...
async function sendChat() {
  ...
  // ✅ FIX: sliding window — keep last 30 messages to avoid token overflow
  ...
  try {
    ...
    try {
      ...
      addAiMsg('Xin lỗi, có lỗi xử lý. Vui lòng thử lại.');
    ...
    thinking.remove();
    addAiMsg('Không thể kết nối LUMEN AI. Vui lòng kiểm tra kết nối.');
  }
  document.getElementById('sendBtn').disabled = false;
  scrollChat();
}
...
function addUserMsg(text) {
  ...
  d.innerHTML = `<div class="msg-avatar user-av">👤</div>
...
function addAiMsg(text) {
  ...
  const d = document.createElement('div');
  d.className = 'msg ai';
  d.innerHTML = `<div class="msg-avatar ai-av">✦</div>
  ...
  d.innerHTML = `<div class="msg-avatar ai-av">✦</div>
    <div class="msg-body">
      <div class="thinking-msg">
        <span>LUMEN đang phân tích</span>
...
function updateSidebar(json) {
  ...
  const labels = {stable:'Ổn định',monitor:'Cần theo dõi',early_support:'Cần hỗ trợ sớm'};
  badge.className = 'risk-badge risk-' + (state.riskLevel === 'early_support' ? 'early' : state.riskLevel);
  document.getElementById('riskText').textContent = labels[state.riskLevel] || 'Ổn định';
...
function updateMiniTimeline() {
  ...
  if (!state.sessionHistory.length) {
    tl.innerHTML = '<div style="font-size:0.78rem;color:var(--cream3);opacity:0.6;text-align:center;padding:8px 0;">Chưa có lịch sử</div>';
    return;
  }
  ...
  tl.innerHTML = state.sessionHistory.slice(0,4).map(h => {
    ...
    return `<div class="tl-item">
      ...
      <div class="tl-preview">${esc(h.text.substring(0,45))}…</div>
      <span class="tl-chip" style="background:${c}20;color:${c}">${h.json.emotional_state_score} điểm</span>
    </div>`;
  }).join('');
}
...
function showResultOverlay(json) {
  ...
  const oe = {stable:'🌿',monitor:'🌤',early_support:'🕯'};
  const titles = {stable:'Bạn đang khá ổn',monitor:'Bạn đang mang điều gì đó',early_support:'Bạn cần được lắng nghe'};
  const rl = json.risk_level || 'stable';
  orb.className = 'result-orb ' + (oc[rl]||'stable');
  orb.textContent = oe[rl]||'🌿';
  ...
  grid.innerHTML = (json.support_recommendation||[]).map(r => {
    const m = recMap[r]||{icon:'💡',title:r,desc:''};
...
function toggleJson() {
  ...
  document.getElementById('jsonArrow').textContent = body.classList.contains('open') ? '▴' : '▾';
}
...
function drawTimeline(range) {
  ...
  canvas.onmousemove = e => {
    ...
    const msgs = [
      'Ngày bình thường',
      'Hôm nay bạn nói "Tôi ổn" 4 lần',
      'Năng lượng thấp bất thường',
      'Điểm cảm xúc ổn định',
      'Phát hiện dấu hiệu kiệt sức nhẹ'
    ];
    tip.innerHTML = `<strong>${data[closest].toFixed(0)} điểm</strong><br>${msgs[closest%msgs.length]}`;
...
function clearData() {
  if (confirm('Xóa tất cả dữ liệu cục bộ? Không thể khôi phục.')) {
    state.sessionHistory = [];
    state.chatHistory = [];
...
function classClick(el, cls, level) {
  const tips = {
    Cao: `Lớp ${cls} đang có xu hướng cảm xúc tốt. Tiếp tục duy trì các hoạt động tích cực.`,
    'Trung bình': `Lớp ${cls} ở mức trung bình. Theo dõi các học sinh có dấu hiệu thu mình.`,
    Thấp: `Lớp ${cls} cần chú ý đặc biệt. Khuyến nghị tư vấn nhóm ngay tuần này.`
  };
  alert('📊 ' + (tips[level] || ''));
}

// ── VOICE ─────────────────────────────────────────────────────
// ✅ FIX: Real Web Speech API voice input
...
function openVoice() {
  if (!SpeechRecognitionAPI) {
    document.getElementById('voiceModalTitle').textContent = '⚠️ Không hỗ trợ Voice';
    document.getElementById('voiceText').innerHTML = 'Trình duyệt không hỗ trợ nhận dạng giọng nói.<br>Hãy thử Chrome hoặc Edge.<br><br>Hít vào thật sâu… thở ra nhẹ nhàng. 🫁';
  ...
  voiceRecognition.onstart = () => {
    isVoiceActive = true;
    document.getElementById('voiceModalTitle').textContent = '🎙️ Đang lắng nghe...';
    document.getElementById('voiceText').innerHTML = '<em style="opacity:.6;font-style:italic;">Hãy nói cảm xúc của bạn...</em>';
    document.getElementById('voiceModal').classList.add('open');
  };
  ...
  voiceRecognition.onresult = (e) => {
    ...
    document.getElementById('voiceText').innerHTML = (final || interim)
      ...
      : '<em style="opacity:.6">Đang nghe...</em>';
    if (final) {
  ...
  voiceRecognition.onerror = (e) => {
    ...
    if (e.error === 'not-allowed') msg = '⚠️ Cần cấp quyền microphone trong trình duyệt.';
    if (e.error === 'network')     msg = '⚠️ Lỗi mạng khi nhận dạng.';
  ...
  voiceRecognition.onend = () => {
    isVoiceActive = false;
    document.getElementById('voiceModalTitle').textContent = '✓ Đã ghi nhận — text đã điền vào chat';
...
const KIDS_STEPS = [
  {
    ...
    type: 'faces',
    options: [
      { emoji: '😄', label: 'Vui lắm!',   desc: 'Hôm nay thật tuyệt', score: 90 },
      { emoji: '🙂', label: 'Ổn thôi',    desc: 'Bình thường bình thường', score: 70 },
      { emoji: '😐', label: 'Bình bình',   desc: 'Không vui cũng không buồn', score: 55 },
      { emoji: '😔', label: 'Hơi buồn',   desc: 'Có chút gì đó nặng nặng', score: 35 },
      { emoji: '😢', label: 'Buồn lắm',   desc: 'Muốn khóc hoặc im lặng', score: 20 },
      { emoji: '😡', label: 'Bực bội',    desc: 'Có gì đó làm mình khó chịu', score: 30 },
    ],
  },
  {
    id: 'color',
    question: 'Nếu cảm xúc của bạn là một màu, đó sẽ là màu gì? 🎨',
    type: 'colors',
    options: [
      { emoji: '☀️', color: '#FFD700', label: 'Vàng', meaning: 'vui vẻ, ấm áp' },
      { emoji: '🌊', color: '#4A90D9', label: 'Xanh dương', meaning: 'bình yên, mát mẻ' },
      { emoji: '🌿', color: '#5CAF6A', label: 'Xanh lá', meaning: 'hy vọng, tươi tắn' },
      { emoji: '🌸', color: '#E88AAA', label: 'Hồng', meaning: 'dịu dàng, mơ mộng' },
      { emoji: '🌌', color: '#7A6AC4', label: 'Tím', meaning: 'suy nghĩ nhiều, lặng lẽ' },
      { emoji: '🌧️', color: '#9AAABB', label: 'Xám', meaning: 'trống rỗng, mệt mỏi' },
    ],
  },
  {
    id: 'energy',
    question: 'Năng lượng hôm nay của bạn là mấy ngôi sao? ⭐',
    type: 'stars',
    max: 5,
  },
  {
    id: 'alone',
    question: 'Có điều gì bạn đang giữ trong lòng mà chưa kể cho ai không? 🤫',
    type: 'choices',
    options: [
      { emoji: '🙅', label: 'Không, tôi hay kể cho người thân', weight: 0 },
      { emoji: '🤔', label: 'Có đôi chút, nhưng không sao',     weight: 1 },
      { emoji: '🤐', label: 'Có, nhưng không biết nói với ai',  weight: 2 },
      { emoji: '😶', label: 'Có, tôi thường giữ mọi thứ một mình', weight: 3 },
    ],
  },
  {
    id: 'help',
    question: 'Bạn muốn làm gì để cảm thấy tốt hơn? 💛',
    type: 'choices',
    options: [
      { emoji: '🎮', label: 'Chơi hoặc vẽ tranh',       weight: 0 },
      { emoji: '🤗', label: 'Được ôm hoặc nói chuyện',  weight: 0 },
      { emoji: '😴', label: 'Nghỉ ngơi và ngủ thêm',    weight: 1 },
      { emoji: '🙏', label: 'Tôi không biết nữa',        weight: 2 },
    ],
  },
];
...
function toggleKidsMode(tabEl) {
  ...
  if (kidsState.active) {
    fab.innerHTML = '❌<div class="kids-fab-tooltip">Thoát chế độ Trẻ em</div>';
    ...
    fab.innerHTML = '🧸<div class="kids-fab-tooltip">Chế độ Trẻ em</div>';
...
function renderKidsScreen() {
  ...
  if (step.type === 'faces') {
    ...
    inputHtml = `<div class="kids-colors">
      ${step.options.map(o => `
        <div class="kids-color-btn" style="background:${o.color}"
          onclick="kidsPickColor(this,'${o.label}')" title="${o.label} — ${o.meaning}">
          ${o.emoji}
        </div>`).join('')}
    </div>
    ...
    const stars = Array.from({length: step.max}, (_,i) =>
      `<span class="kids-star" onclick="kidsPickStar(${i+1})" data-val="${i+1}">⭐</span>`
    ).join('');
  ...
  wrap.innerHTML = `
    ...
    <div class="kids-card">
      <div class="kids-step-label">
        <span>✦</span> Câu ${kidsState.step + 1} / ${KIDS_STEPS.length} <span>✦</span>
      </div>
...
function kidsPickColor(el, label) {
  ...
  if (lbl && opt) lbl.textContent = `${opt.label} — ${opt.meaning} ${opt.emoji}`;
  document.getElementById('kidsNextBtn').disabled = false;
}
...
function kidsPickStar(val) {
  ...
  const labels = ['','Mệt lắm rồi 😴','Hơi mệt 😔','Bình thường 🙂','Khá ổn 😊','Tràn đầy năng lượng! 🚀'];
...
function renderKidsResult(wrap) {
  ...
  const configs = {
    stable: {
      emoji: '🌟', title: 'Bạn đang rất ổn!',
      msg: `Tuyệt vời! Bạn đang có một ngày khá tốt. Hãy tiếp tục vui vẻ và chia sẻ nụ cười với mọi người nhé! ${feeling ? feeling.emoji : '😊'}`,
      color: '#5CAF6A', bg: 'rgba(92,175,106,0.12)',
    },
    monitor: {
      emoji: '🌤', title: 'Bạn đang ổn — nhưng có chút gì đó…',
      msg: `Không sao cả! Đôi khi trong lòng mình có điều chưa nói được thì cũng bình thường. Hãy tìm một người lớn bạn tin tưởng và kể nhé! 🤗`,
      color: '#D4A84B', bg: 'rgba(212,168,75,0.12)',
    },
    early_support: {
      emoji: '🫂', title: 'Bạn cần được ôm và lắng nghe',
      msg: `Cảm ơn bạn đã chia sẻ! Bạn không cần phải một mình với cảm xúc này. Hãy kể với bố/mẹ, thầy/cô, hoặc người bạn thật sự tin tưởng nhé — họ sẽ luôn ở đây cho bạn. 💛`,
  ...
  const activities = [
    { icon: '🎨', label: 'Vẽ tranh cảm xúc' },
    { icon: '📖', label: 'Đọc truyện yêu thích' },
    { icon: '🤸', label: 'Vận động 10 phút' },
    { icon: '🫁', label: 'Thở sâu 5 lần' },
  ];

  const starsDisplay = '⭐'.repeat(energy) + '☆'.repeat(5 - energy);
    ...
    <div class="kids-result-card">
      ...
      <div style="display:flex;gap:16px;justify-content:center;flex-wrap:wrap;margin-bottom:24px;">
        <div style="text-align:center;padding:12px 20px;background:${cfg.bg};border:2px solid ${cfg.color}33;border-radius:16px;">
          <div style="font-size:0.7rem;letter-spacing:0.12em;text-transform:uppercase;color:${cfg.color};margin-bottom:4px;">Cảm xúc</div>
          <div style="font-size:2rem;">${kidsState.answers.feeling || '😊'}</div>
        </div>
        <div style="text-align:center;padding:12px 20px;background:var(--navy3);border:2px solid var(--border2);border-radius:16px;">
          <div style="font-size:0.7rem;letter-spacing:0.12em;text-transform:uppercase;color:var(--cream3);margin-bottom:4px;">Năng lượng</div>
          <div style="font-size:1rem;letter-spacing:0.05em;">${starsDisplay}</div>
        </div>
      </div>

      <div style="font-size:0.75rem;letter-spacing:0.15em;text-transform:uppercase;color:var(--cream3);margin-bottom:12px;">Gợi ý hoạt động 💡</div>
      ...
      <div style="background:rgba(232,138,170,0.1);border:2px solid rgba(232,138,170,0.3);border-radius:18px;padding:18px;margin-bottom:20px;display:flex;gap:14px;align-items:flex-start;">
        ...
        <div>
          <div style="font-size:0.9rem;color:#E88AAA;font-weight:600;margin-bottom:4px;">Nói chuyện với người lớn bạn tin nhé!</div>
          <div style="font-size:0.82rem;color:var(--cream2);line-height:1.6;">Bố/mẹ, thầy/cô, hoặc gọi đường dây hỗ trợ: <strong style="color:#E88AAA;">1800 599 920</strong></div>
        </div>
      </div>` : ''}
...
// ── NAVIGATION (patched to include kids screen) ───────────────
